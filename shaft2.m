...
B2=sym('B2_%d',[1 3]);
syms T1
F_value={[-1.79,2.52,6.7], [1.12,-0.58,3.48]}; % in KN****************
F_location={[90,-46.6,0],[250,89.7,0]}; % in mm*******************
n_=size(F_value);
n=n_(2);
for i=1:n
    eq1=eq1 + F_value{i};
...
solution=solve([eq1,eq2],[B1,B2,T1]);
 %solution.B1_1=B2_1;solution.B2_1=0;  %*****************************
...
title("Moment 2(x) in N.m");
figure
plot(T(:,1) , T(:,2),'r');
xlabel('position on shaft in mm');
ylabel('Torque in N.m');
title("Torque 2(x) in N.m");
